...
/**
 * Created by zzg on 2017/12/27.
 */

public abstract class MaApplicationLike extends DefaultApplicationLike {
    private static final String TAG = "MaApplicationLike";
    private static MaApplicationLike sInstance;
    private ArrayList<PriorityLogicWrapper> mLogicList;
    private HashMap<String, ArrayList<PriorityLogicWrapper>> mLogicClassMap;

    public MaApplicationLike(Application application, int tinkerFlags, boolean tinkerLoadVerifyFlag, long applicationStartElapsedTime, long applicationStartMillisTime, Intent tinkerResultIntent) {
    ...
    protected void startWideRouter(){
        if (needMultipleProcess()) {
            ...
            Intent intent = new Intent(getApplication(), WideRouterConnectService.class);
    ...
    private void dispatchLogic(){
        if (null != mLogicClassMap) {
            mLogicList = mLogicClassMap.get(ProcessUtil.getProcessName(getApplication(), ProcessUtil.getMyProcessId()));
        }
    }
    ...
    public static MaApplicationLike getMaApplicationLike(){
        return sInstance;
    }
}
